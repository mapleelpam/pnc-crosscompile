...
THRIFT_CONFIG := --enable-gen-c_glib=no --with-java=no --with-python=no --with-csharp=no --with-erlang=no --with-libevent=no --with-zlib=no --enable-gen-cpp=no
...
thrift-build:
	if [ ! -f $(THRIFT_SRC)/config.h ]; then \
			cd $(THRIFT_SRC) && chmod +x ./configure && ac_cv_va_copy=1 sh ./configure $(THRIFT_CONFIG) --host=$(HOST) --prefix=/; \
	fi
